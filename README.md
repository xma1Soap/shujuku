...
### 正文替换匹配失败修复
**问题描述**：用户反馈点击应用后部分替换没有应用上。

**问题原因**：
- 原代码使用 `indexOf` 精确匹配，如果AI返回的 `original` 与实际正文有细微差异（如空格、换行、标点等），就会导致替换失败
- 没有日志记录替换失败的情况，用户无法知道哪些优化项未能应用

**修复内容**：
1. **多策略匹配**：
   - 策略1：精确匹配（原有逻辑）
   - 策略2：去除首尾空白后匹配
   - 策略3：忽略多余空白匹配（使用Levenshtein相似度算法）
...
3. **新增函数**：
   - [`calculateSimilarity_ACU()`](index.js:2330) - 计算两个字符串的相似度（0-1）

**修改位置**：
- [`applyOptimizations_ACU()`](index.js:2253-2328) - 重写替换逻辑，支持多策略匹配
