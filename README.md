...
## 2026-04-08 更新（补充修复：iPad 端“数据管理”页面按钮与卡片布局溢出）

### 功能描述

1. 修复 iPad / 小平板宽度下“数据管理”页面按钮组容易飞出容器的问题：为数据管理按钮组补充 `min-width: 0`，并在 `<=1100px` 下将 3 列按钮组降为 2 列，同时让按钮改为自适应高度、减小横向内边距并允许换行，避免长文案按钮把网格撑出屏幕；对应实现位于 [`index.js`](index.js:22917)-[`index.js`](index.js:22924) 与 [`index.js`](index.js:23291)-[`index.js`](index.js:23301)。
2. 同步收紧“数据管理”页内部其他容易在 iPad 宽度下撑开的区域：模板预设双卡片区在 `<=1100px` 下折叠为单列，预设工具按钮允许换行伸缩，数据隔离输入区改为纵向堆叠，减少整页横向溢出风险；对应实现位于 [`index.js`](index.js:23266)-[`index.js`](index.js:23290)、[`index.js`](index.js:24198) 与 [`index.js`](index.js:24205)。
3. 已执行 `node --check index.js`，当前脚本通过语法检查。

### 修改位置

| 文件 / 场景 | 行号区间 | 说明 |
|------|------|------|
| 数据管理按钮组最小宽度约束 | 22917-22924 | 为 `.acu-data-mgmt-buttons` 补充 `min-width: 0`，避免 grid 容器被子项反向撑开 |
| iPad 数据管理按钮响应式折叠 | 23291-23301 | `<=1100px` 下将 3 列按钮组改为 2 列，并允许按钮换行、自适应高度与更紧凑内边距 |
| iPad 数据管理页内部卡片 / 工具栏响应式收缩 | 23266-23290 | 模板预设区改单列、工具按钮允许换行、数据隔离输入区改为纵向堆叠 |
| 数据管理结构类名标记 | 24198、24205 | 为模板预设网格和数据隔离行添加专用类名，便于定向做平板断点覆盖 |
...
## 2026-04-08 更新（补充修复：手机端“剧情推进”板块溢出）

### 功能描述

1. 修复手机端“剧情推进”页横向溢出问题：为剧情推进头部、预设双卡片区、任务编辑双栏区补充专用类名与 `min-width: 0` 约束，避免内部 grid/flex 子项把容器撑出屏幕；对应实现位于 [`index.js`](index.js:23114)-[`index.js`](index.js:23121) 与 [`index.js`](index.js:24444)-[`index.js`](index.js:24533)。
2. 在 `<=768px` 下让剧情推进头部自动换行；在 `<=520px` 下将预设区与任务区强制折叠为单列，并让提示词工具栏纵向堆叠，避免手机宽度下两栏布局继续超屏；对应实现位于 [`index.js`](index.js:23267)-[`index.js`](index.js:23356)。
3. 已执行 `node --check index.js`，当前脚本通过语法检查。
...
| 文件 / 场景 | 行号区间 | 说明 |
|------|------|------|
| 剧情推进容器最小宽度约束 | 23114-23121 | 为头部、预设区、任务区及其子项补充 `min-width: 0`，防止 grid/flex 子项撑破移动端容器 |
| 剧情推进手机端布局折叠 | 23267-23356 | 手机端头部换行、预设区单列、任务区单列、提示词工具栏改为纵向堆叠 |
| 剧情推进结构类名标记 | 24444-24533 | 为剧情推进头部、预设区、任务双栏区加入专用类名，便于移动端定向覆盖 |
